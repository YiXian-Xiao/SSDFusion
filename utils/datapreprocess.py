...
def Im2Patch(img, win, stride=1, dtype=np.float32):
    k = 0
    endc = img.shape[0]
    endw = img.shape[1]
    endh = img.shape[2]
...
IR_files = sorted(get_img_file(r"data/dataset/FMB/train/ir"))
VIS_files = sorted(get_img_file(r"data/dataset/FMB/train/vi"))
LABEL_files = sorted(get_img_file(r"data/dataset/FMB/train/Segmentation_labels"))
...
for i in tqdm(range(len(IR_files))):
    ...
    for ii in range(I_IR_Patch_Group.shape[-1]):
        ...
        if not (bad_IR or bad_VIS or bad_LABEL):
            ...
            h5_label.create_dataset(str(train_num), data=avl_LABEL,
                                  dtype=avl_LABEL.dtype, shape=avl_LABEL.shape)
            train_num += 1
